...
export default class PGPool {
  ...
  query(cUser: ShopKeeper, sqlText: string, params: any[] = [], callback: Callback) {
    if (!cUser) {
    ...
    this.pool.connect(function (err: Error, client: any, done: any) {
      ...
      client.query(`SET SESSION postgres.username = '${cUser.name}'`, [], function (err: Error) {
        if (err) {
          done()
          return callback(err)
        } else {
  ...
  async aquery(cUser: ShopKeeper, sqlText: string, params: any[] = []): Promise<pg.QueryResult<any>> {
    const client = await this.pool.connect()
    try {
      await client.query(`SET SESSION postgres.username = '${cUser.name}'`, [])
